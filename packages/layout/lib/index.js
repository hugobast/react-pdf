...
const splitText = (node, height) => {
    ...
    // remainder.
    const remainingLineHeight = heightAtLineIndex(node, node.lines.length) - currentHeight;
    const nextPaddingBottom = typeof node.box?.paddingBottom === 'number' ? node.box.paddingBottom : 0;
    const minH = typeof node.style?.minHeight === 'number' ? node.style.minHeight : 0;
 ...
 */
const shouldBreakOptimized = (child, furthestEndOfNonFixedFuture, height, hasNonFixedPrevious) => {
    if (isFixed(child))
        return false;
    const shouldSplit = height < child.box.top + child.box.height;
    const canWrap = getWrap(child);
    const afterMinPresenceAhead = getEndOfMinPresenceAhead(child);
    const endOfPresence = furthestEndOfNonFixedFuture === null
 ...
 */
const computeSuffixFurthestEnd = (nodes) => {
...
const collectFixedIndices = (nodes) => {
    const indices = [];
    const length = nodes.length;
...
const isAbsolutePositioned = (node) => node.style?.position === 'absolute';
...
const getOutsetHeight = (node) => (node.box?.marginTop || 0) +
    (node.box?.height || 0) +
    (node.box?.marginBottom || 0);
const splitNodes = (height, contentArea, nodes, isRowLayout = false, rowGap = 0, initialTop = 0) => {
    const currentChildren = [];
    const nextChildren = [];
    const suffixFurthestEnd = computeSuffixFurthestEnd(nodes);
    const fixedIndices = collectFixedIndices(nodes);
    const length = nodes.length;
    const pushFutureFixed = (target, afterIndex) => {
        for (const idx of fixedIndices) {
            if (idx > afterIndex)
                target.push(nodes[idx]);
        }
    };
    ...
    const placeOnNextPage = (node) => {
        if (isRowLayout) {
            return Object.assign({}, node, {
                box: Object.assign({}, node.box, { top: node.box?.top || 0 }),
            });
        }
        if (placedFlowCount > 0)
            cumNonFixedNextHeight += rowGap;
        placedFlowCount += 1;
        ...
        const newTop = Math.max(cumFixedHeight, initialTop) + cumNonFixedNextHeight + marginTop;
    ...
    for (let i = 0; i < length; i += 1) {
        ...
        if (!fitsInsidePage && !getWrap(child)) {
            currentChildren.push(child);
            nextChildren.push(...adjustRemaining(i + 1));
            warnUnavailableSpace(child);
            break;
        }
        const shouldBreak = shouldBreakOptimized(child, suffixFurthestEnd[i], height, hasNonFixedPrevious);
        if (shouldBreak) {
        ...
        if (shouldSplit) {
            ...
            if (nextChild)
                nextChildren.push(placeOnNextPage(nextChild));
...
const getRowGap = (node) => {
    const gap = node.style?.rowGap;
...
const splitChildren = (height, contentArea, node) => {
    ...
    return splitNodes(availableHeight, contentArea, children, isRowLayout, getRowGap(node));
};
...
const computeActualNextDimensions = (nextChildren, parent, rowGap = 0) => {
    ...
    const flow = nextChildren.filter((c) => !isFixed(c) && !isAbsolutePositioned(c));
    const fd = parent.style
        ?.flexDirection;
    ...
    if (isRow) {
        ...
        const updatedChildren = nextChildren.map((c) => {
            if (isFixed(c) || isAbsolutePositioned(c))
                return c;
...
const splitView = (node, height, contentArea) => {
    ...
    if (node.style?.height == null) {
        const { actualH, updatedChildren } = computeActualNextDimensions(nextChildren, nextNode, // next-half: style.paddingTop=0, style.paddingBottom=original
        getRowGap(nextNode));
        const next = assingChildren(updatedChildren, Object.assign({}, nextNode, {
...
const splitPage = (page, pageNumber, fontStore, yoga, isSynthesized = false) => {
    const wrapArea = getWrapArea(page) - (isSynthesized ? SYNTHESIZED_WRAP_MARGIN : 0);
    const contentArea = getContentArea(page);
    const dynamicPage = resolveDynamicPage({ pageNumber }, page, fontStore, yoga);
    const height = page.style.height;
    const pagePaddingTop = getPadding(page).paddingTop || 0;
    const [currentChilds, nextChilds] = splitNodes(wrapArea, contentArea, dynamicPage.children, false, getRowGap(page), pagePaddingTop);
    const relayout = (node) => 
...
const paginate = (page, pageNumber, fontStore, yoga) => {
    ...
    while (nextPage !== null) {
        splittedPage = splitPage(nextPage, pageNumber + pages.length, fontStore, yoga, true);
